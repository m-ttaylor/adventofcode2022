...
def findIfSubsetPresent(elfA: tuple[str, str], elfB: tuple[str, str]) -> bool:
    perms = ((elfA[0], elfA[1], elfB[0], elfB[1]), (elfB[0], elfB[1], elfA[0], elfA[1]))
...
def findIfOverlapPresent(elfA: tuple[str, str], elfB: tuple[str, str]) -> bool:
    ...
    for lowerA, higherA, lowerB, higherB in perms:
        ...
        if int(lowerA) in range(int(lowerB), int(higherB) + 1) or int(higherA) in range(
            int(lowerB), int(higherB) + 1
        ):
            return True
...
def findBadElfPairs(data: list, mode: str) -> int:
    ...
    for elfPair in data:
        ...
        hasSubset = (
            findIfSubsetPresent(elfA, elfB)
            if mode == "subset"
            else findIfOverlapPresent(elfA, elfB)
            if mode == "overlap"
            else exit(1)
        )
